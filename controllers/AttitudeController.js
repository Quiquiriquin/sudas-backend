...
const AttitudeController = {
  create: async (req, res) => {
    try {
      const { name } = req.body;
      const newSkill = await models.attitude.create({
        name
      });
  ...
  list: async (req, res) => {
    try {
      const verbs = await models.attitude.findAll();
  ...
  get: async (req, res) => {
    try {
      const { id } = req.params;
      const verb = await models.attitude.findOne({
        where: {
          id,
        },
      });
  ...
  update: async (req, res) => {
    try {
      ...
      const updatedVerb = await models.attitude.update(
        {
          name,
        },
        {
